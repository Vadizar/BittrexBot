...
MARKETS = [
    'USDT-BTC', 'USDT-ETH', 'USDT-BCC', 'USDT-BTG', 'USDT-XRP',
    'USDT-LTC', 'USDT-DASH', 'USDT-XMR', 'USDT-ETC', 'USDT-NEO',
    'USDT-ZEC', 'USDT-OMG'
]

CAN_SPEND = 35 # Сколько USDT готовы вложить в покупку
MARKUP = 0.001 # 0.001 = 0.1% - Какой навар со сделки хотим получать

STOCK_FEE = 0.0025 # Какую комиссию берет биржа

ORDER_LIFE_TIME = 1 # Через сколько минут отменять неисполненный ордер на покупку 0.5 = 30 сек.
...
def get_ticks(market):
    ...
    res = requests.get("https://bittrex.com/Api/v2.0/pub/market/GetTicks?marketName=" + market + "&tickInterval=thirtyMin")
                ...
                for order in orders_info:
                    if orders_info[order]['order_type'] == 'buy':
                        if orders_info[order]['order_filled']: # если ордер на покупку был выполнен
                            ...
                            if USE_MACD:
                                macd_advice = get_macd_advice(chart_data=get_ticks(market)) # проверяем, можно ли создать sell
                                if macd_advice['trand'] == 'BEAR' or (macd_advice['trand'] == 'BULL' and macd_advice['growing']):
                                    log(market, 'Для ордера %s не создаем ордер на продажу, т.к. ситуация на рынке неподходящая' % order)
                                else:
